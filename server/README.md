...
offcat 后端服务，为求职者提供 AI 驱动的简历诊断、模拟面试、JD 匹配、简历润色、薪资谈判模拟、社区交流、用户自定义 AI 引擎等能力，附带完整的会员体系和管理后台。
...
服务启动后访问 `GET /actuator/health` 验证。
...
完整公开 API 导航见 [../docs/api/API_INDEX.md](../docs/api/API_INDEX.md)。本节保留后端模块级说明，具体请求体和响应体以 Controller、DTO 与 API 文档为准。
...
| `/api/diagnostic/**`, `/api/stats`, `/api/version-logs/**` | 公开 |
| `/actuator/health`, `/actuator/info` | 公开 |
| `/actuator/**` | 需要 `ROLE_ADMIN` |
