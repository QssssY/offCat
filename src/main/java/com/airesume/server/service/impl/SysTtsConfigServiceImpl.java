...
public class SysTtsConfigServiceImpl implements SysTtsConfigService {
    ...
    public AdminTtsConfigResponse saveConfig(AdminTtsConfigRequest request) {
        ...
        boolean edgeProvider = isEdgeProvider(config.getTtsProvider());
        ...
        if (!shouldValidateCompleteFields) {
            ...
            String apiKey = edgeProvider ? null : normalizeApiKeyForSave(request, existing);
    ...
    @Override
    public TtsAudioResult previewVoiceAudio(AdminTtsConfigRequest request) {
        return userTtsConnectivityTestService.previewVoiceAudio(buildConnectivityRequest(request));
    }
    ...
    public UserTtsDiscoveryResponse discover(AdminTtsConfigRequest request) {
        ...
        String apiKey = isEdgeProvider(provider) ? "" : normalizePlainApiKey(request, sysTtsConfigMapper.selectCurrent());
    ...
    private UserTtsConnectivityTestRequest buildConnectivityRequest(AdminTtsConfigRequest request) {
        ...
        String provider = normalizeProvider(request.getTtsProvider());
        normalized.setApiKey(isEdgeProvider(provider) ? "" : normalizePlainApiKey(request, existing));
    ...
    private ResolvedTtsConfig buildResolvedConfig(SysTtsConfig config) {
        ...
        // EdgeTTS 无用户密钥，系统兜底解析时跳过解密和 Key 必填判断。
        String apiKey = isEdgeProvider(provider) ? "" : trimToNull(aiCredentialCrypto.decrypt(config.getApiKey()));
        if (!isEdgeProvider(provider) && apiKey == null) {
            return null;
        }
    ...
    private AdminTtsConfigResponse buildResponse(SysTtsConfig config) {
        ...
        return AdminTtsConfigResponse.builder()
                ...
                .apiKey(isEdgeProvider(config.getTtsProvider())
                        ? null
                        : maskApiKey(trimToNull(aiCredentialCrypto.decrypt(config.getApiKey()))))
    ...
    private boolean isConfigured(SysTtsConfig config) {
        ...
        boolean hasCommonFields = trimToNull(config.getBaseUrl()) != null
        ...
        return hasCommonFields && trimToNull(config.getApiKey()) != null;
    }
    ...
    private boolean isEdgeProvider(String provider) {
        return TtsProviderConstants.PROVIDER_EDGE.equals(normalizeProvider(provider));
    }
