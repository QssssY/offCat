...
public class ResumeDiagnosisConsumer {
    ...
    public void handleResumeDiagnosisTask(ResumeDiagnosisMessage message) {
        ...
        try {
            // 前置检查：任务是否仍处于待处理状态（防止重复处理已被手动标记失败的任务）
            ...
            if (currentStatus != ResumeDiagnosisConstants.STATUS_PENDING) {
                log.warn("任务已非待处理状态(status={}), 跳过处理, taskId: {}", currentStatus, taskId);
                return;
            }
            ...
            // 清理控制字符，防止MySQL JSON列报Invalid encoding
            ...
            // 终极兜底：防止Error或未预期的Throwable导致Consumer容器崩溃
            log.error("简历诊断严重异常（Throwable），taskId: {}", taskId, t);
            try {
                refundQuotaIfNeeded(message.getUserId(), taskId);
            } catch (Exception ignored) {}
            markTaskFailed(taskId, "系统严重异常: " + t.getMessage());
        }
    }
